...
- `lib/server/tenant-cutover.ts` e `004-maintenance.sql`: ativação somente após estrutura e conciliação verificadas; despacho bloqueado em preparação/manutenção. O worker revê autorização, revogação e MFA durante o corte. Testes reais cobrem falhas antes/depois do commit, repetição e consulta operacional com contexto central. O CLI e o acompanhamento foram acrescentados em 26/09; não há uso em produção.
- O importador recusa origem operacional v2 já utilizada e fotos sem registro de arquivo verificado. A importação de fotos por pacote foi acrescentada em 26/09; a origem v2 continua pendente.
...
- O ensaio de backup ampliado para bytes ficou pendente em 21/09: `pg_dump`, `pg_restore` e `psql` foram impedidos de carregar `libpq.dll` pelo Smart App Control (evento 3077, código `0xc0e90002`). Essa pendência foi resolvida pelos ensaios de 26/09 descritos abaixo.
...
## Fotos antigas e acompanhamento — avanço em 26/09

- Pacote privado de fotos a partir de exportação local: escopo exato das referências congeladas, limites, formato, SHA-256, manifesto escrito por último e recusa de sobrescrita. O leitor recusa adulteração e arquivos ausentes; os caminhos originais não viram caminhos locais no leitor.
- Importação de bytes e metadados na mesma transação dos produtos/estoque/financeiro. Preserva referências e autores dos produtos; identifica o ADM importador separadamente. Repetição verifica origem, pacote e destino, inclusive após nova autorização por outro ADM.
- Comandos privados `prepare-legacy-photos.mjs` e `cutover-tenant.mjs`, com TLS obrigatório e credenciais somente no ambiente. O banco vem da empresa do job central. Ajuda e erros não expõem segredos.
- Acompanhamento integrado a **Empresas → Verificar banco**: etapas, empresa/UUID, justificativa, confirmação de interrupção, reautenticação e atualização de status/contagens. Consulta e solicitação usam RPCs autorizadas no servidor e auditoria. A migração central de corte continua não aplicada.
- Passaram: testes PostgreSQL de importação com/sem fotos, rollback após inserir bytes, hashes/referências, pacote de outra empresa, adulteração local/no destino, renovação por outro ADM, worker completo/ativação/leitura restrita, cadastro central; testes do pacote, comandos e handlers HTTP; TypeScript, lint direcionado e build Next.js.
- Navegador com componente real/API fictícia: formulário bloqueado sem confirmação, solicitação registrada, estado atualizado e contexto limpo ao trocar empresa. DOM em 390 px sem transbordamento horizontal; nenhuma operação de produção. Não comprova login/MFA real.
- Também passaram os ensaios nativos de backup/restauração: PNG e pacote 3MF recuperados byte a byte após alteração controlada, rollback após falha, cópia anterior, outra empresa preservada e repetição. O teste de manutenção central/MFA e retomada também passou. Os executáveis oficiais voltaram a funcionar; nenhuma proteção do Windows foi alterada pelo agente.
- Ainda pendentes: interrupção/drenagem das gravações do Storage e exportação de produção; demais itens abaixo. O pacote local não prova a atualidade da exportação. Nenhuma empresa ou arquivo de produção foi migrado nesta etapa.
...
2. Concluir a implantação da arquitetura exclusiva: provisionador, motor, API, importador legado, conciliação e worker de corte passaram em PostgreSQL real; faltam a janela/exportação real do Storage, execução operacional em produção e origem v2 quando aplicável. O registro de arquivos, importação por pacote e acompanhamento no painel foram implementados e testados localmente. Cadastros empresariais/vínculos e o cliente já foram adaptados no branch; falta instalar a migração central em conjunto com a publicação e testar uma sessão real. Configurar credenciais de produção e verificar CONNECT em outras bases, sem retirar privilégios internos do Supabase às cegas. Não aplicar a migração operacional compartilhada como substituto.
