...
function AdminPanel({db}:{db:SupabaseClient}){
 ...
 function formFields():FieldSpec[]{if(!modal)return [];
  ...
  if(modal.type==='permission.save')return [{key:'scope',label:'Nível',options:[['company','Empresa'],['role','Perfil'],['user','Usuário']]},{key:'subject',label:'Destinatário',required:true,value:'*',hint:'Empresa: *. Perfil: admin, sales, purchases, stock, finance ou read. Usuário: ID do vínculo.'},{key:'module',label:'Módulo',options:['overview','sales','purchases','catalog','stock','finance','production','settings'].map(x=>[x,x])},{key:'action',label:'Ação',options:['available','visible','read','create','edit','delete','approve','cancel','reverse','export'].map(x=>[x,x])},{key:'allowed',label:'Regra',options:[['false','Bloquear'],['true','Permitir']]},reason];
