...
import {createTenantBackup,inspectBackup,verifyTenantRecovery} from '../lib/server/tenant-backup.ts';
...
try{
 ...
 for(const company of companyIds)await provisionTenant({companyId:company,runtimePassword:randomBytes(36).toString('base64url'),maintenance:admin,connectMaintenanceDatabase:database=>postgres({...base,database}),migrations});
 ...
 if(backup){for(const name of ['archive.enc','manifest.json','manifest.json.tmp'])await unlink(join(root,backup.id,name)).catch(()=>{});await rmdir(join(root,backup.id)).catch(()=>{});}
