...
import {readFile,mkdtemp,writeFile,rm,mkdir,access} from 'node:fs/promises';
...
import {openLegacyPhotoBundle,prepareLegacyPhotoBundle} from '../lib/server/legacy-photo-bundle.ts';
...
const opened=[];
const withPhotos=process.argv.includes('--photos');let bundleDirectory,bundleRoot,exportDirectory;
try {
 ...
 let bundle,photo,photoBytes;
 if(withPhotos){
  photoBytes=Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jZ1kAAAAASUVORK5CYII=','base64');
  photo={name:owner+'/foto-legada.png',filename:'foto-legada.png',mime:'image/png',size_bytes:photoBytes.length,sha256:createHash('sha256').update(photoBytes).digest('hex')};
  // Source is a legacy fixture, where the new asset-reference trigger did not exist.
  await source`drop trigger product_asset_reference on public.products`;
  await source`update public.products set image_path=${photo.name} where id=${product}`;
  bundleRoot=await mkdtemp(join(resolve('work'),'import-photo-test-'));
  bundleDirectory=join(bundleRoot,'bundle');exportDirectory=join(bundleRoot,'export');
  await mkdir(join(exportDirectory,owner),{recursive:true});
  await writeFile(join(exportDirectory,...photo.name.split('/')),photoBytes);
 }
 const copy=()=>importLegacyCompany(source,target,job,bundle);
 ...
 if(withPhotos)await assert.rejects(prepareLegacyPhotoBundle(source,job,exportDirectory,bundleDirectory),e=>e.code==='SOURCE_NOT_FROZEN');
 ...
 if(withPhotos){
  await assert.rejects(prepareLegacyPhotoBundle(source,job,exportDirectory,join(exportDirectory,'nested-bundle')),e=>e.code==='ASSET_BUNDLE_INVALID');
  // An incomplete export leaves no usable manifest and never touches the database.
  await rm(join(exportDirectory,...photo.name.split('/')));
  const incomplete=join(bundleRoot,'incomplete');
  await assert.rejects(prepareLegacyPhotoBundle(source,job,exportDirectory,incomplete));
  await assert.rejects(access(join(incomplete,'manifest.json')));
  await writeFile(join(exportDirectory,...photo.name.split('/')),photoBytes);
  const prepared=await prepareLegacyPhotoBundle(source,job,exportDirectory,bundleDirectory);
  assert.equal(prepared.company,company);assert.equal(prepared.photos,1);
  await assert.rejects(prepareLegacyPhotoBundle(source,job,exportDirectory,bundleDirectory),e=>e.code==='EEXIST');
  bundle=await openLegacyPhotoBundle(prepared.manifest,company);
  await assert.rejects(importLegacyCompany(source,target,job),e=>e.code==='ASSET_BUNDLE_REQUIRED');
  await assert.rejects(importLegacyCompany(source,target,job,{...bundle,company:otherCompany}),e=>e.code==='ASSET_BUNDLE_REQUIRED');
  await assert.rejects(importLegacyCompany(source,target,job,{...bundle,photos:[...bundle.photos,{...photo,name:'unreferenced.png'}]}),e=>e.code==='ASSET_BUNDLE_REQUIRED');
  await writeFile(join(bundleDirectory,photo.sha256+'.bin'),Buffer.alloc(photo.size_bytes,1));
  await assert.rejects(copy(),e=>e.code==='ASSET_BUNDLE_INVALID');
  await writeFile(join(bundleDirectory,photo.sha256+'.bin'),photoBytes);
  await target.unsafe("create function tenant.fail_photo_import() returns trigger language plpgsql as $$begin raise exception 'Injected import failure';end$$;create trigger fail_photo_import before insert on public.business_units for each row execute function tenant.fail_photo_import()");
  await assert.rejects(copy(),e=>/Injected import failure/.test(e.message));
  for(const table of ['tenant.assets','tenant.uploads','tenant.actors','tenant.reconciliations','public.products'])assert.equal((await target`select count(*)::int as n from ${target(table)}`)[0].n,0);
  await target.unsafe('drop trigger fail_photo_import on public.business_units;drop function tenant.fail_photo_import()');
 }
 const report=await copy();
 assert.equal(report.source_digest,report.target_digest);
 assert.deepEqual(report.counts,{businesses:1,products:1,entries:3,movements:1,...(withPhotos?{photos:1}:{})});
 ...
 assert.deepEqual(await copy(),report);
 if(withPhotos){
  const asset=(await target`select content,sha256,created_by,name from tenant.assets`)[0];
  assert.deepEqual(asset.content,photoBytes);assert.equal(asset.sha256,photo.sha256);assert.equal(asset.created_by,requester);assert.equal(asset.name,photo.name);
  assert.equal((await target`select owner_id from public.products where id=${product}`)[0].owner_id,owner);
  assert.equal((await other`select count(*)::int as n from tenant.assets`)[0].n,0);
  await writeFile(join(bundleDirectory,photo.sha256+'.bin'),Buffer.alloc(photo.size_bytes,1));
  await assert.rejects(copy(),e=>e.code==='ASSET_BUNDLE_INVALID');
  await writeFile(join(bundleDirectory,photo.sha256+'.bin'),photoBytes);
  await target`update tenant.assets set content=${Buffer.alloc(photo.size_bytes,2)}`;
  await assert.rejects(copy(),e=>e.code==='RECONCILIATION_FAILED');
  await target`update tenant.assets set content=${photoBytes}`;
  assert.deepEqual(await copy(),report);
  // Renewing authorization with a different ADM preserves the first importer.
  await source`update erp_control.cutovers set requested_by=${owner} where id=${job}`;
  assert.deepEqual(await copy(),report);
  assert.equal((await target`select created_by from tenant.assets`)[0].created_by,requester);
 }
 ...
 await source`update erp_control.companies set data_location='migrating' where id=${company}`;
 await assert.rejects(copy(),e=>e.code==='SOURCE_CHANGED');
 ...
 if(withPhotos)console.log('PASS: fotos legadas preservadas por hash/referência; importador distinto do autor original; pacote obrigatório e escopo exato; rollback inclui bytes e metadados; repetição detecta alteração no arquivo local ou no banco.');
} finally {
 ...
 if(bundleRoot){assert.equal(dirname(resolve(bundleRoot)),resolve('work'));await rm(bundleRoot,{recursive:true,force:true});}
}
